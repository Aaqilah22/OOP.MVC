...
```
Penjelasan :
Kelas BaseModel adalah kerangka abstrak dalam Java yang dirancang untuk mengelola operasi database menggunakan JDBC. Dengan memanfaatkan konsep generik (<T>) dan metode abstrak, kelas ini memastikan bahwa setiap kelas turunan memiliki struktur yang seragam untuk melakukan operasi CRUD (Create, Read, Update, Delete) pada database.
...
```
Penjelasan :
Kelas Database adalah implementasi standar untuk mengelola koneksi ke database MySQL dalam aplikasi Java, memudahkan pengembang untuk membuat koneksi dengan pendekatan yang aman, modular, dan terstruktur.
...
```
penjelasan :
nterface RowMapper adalah alat penting dalam pengembangan aplikasi berbasis database. Dengan menyediakan kerangka untuk memetakan ResultSet ke objek, interface ini meningkatkan modularitas, konsistensi, dan reusability dalam kode Anda.
...
```
penjelasan : 
Kelas MahasiswaController adalah pengendali utama untuk fitur manajemen Mahasiswa. Dengan mengelola interaksi antara tampilan (FormMahasiswa), model (MahasiswaModel), dan database, kelas ini menyediakan antarmuka yang user-friendly, modular, dan mudah diperluas. Kombinasi dengan arsitektur MVC memastikan aplikasi terorganisasi dengan baik dan fleksibel untuk perubahan di masa depan.
...
```
penjelasan :
Kelas NilaiController adalah pengendali utama untuk fitur manajemen nilai mahasiswa. Dengan mengelola interaksi antara tampilan (FormNilai), model (NilaiModel), dan database, kelas ini menyediakan antarmuka yang mudah digunakan untuk mengelola nilai mahasiswa. Fitur validasi data dan konfirmasi operasi menjadikan aplikasi lebih aman dan andal. Penggunaan MVC memastikan kode terorganisasi dengan baik, modular, dan mudah di-maintain.
...
```
Penjelasan : 
Kelas Mahasiswa dalam Java digunakan untuk menyimpan informasi tentang mahasiswa, seperti ID, NIM, nama, jurusan, dan angkatan. Kelas ini memiliki dua konstruktor: satu default tanpa parameter dan satu lagi untuk menginisialisasi nilai variabel. Selain itu, kelas ini menyediakan metode getter dan setter untuk masing-masing atribut, memungkinkan akses dan perubahan data mahasiswa.
...
```
Penjelasan :
Kelas MahasiswaModel adalah turunan dari **BaseMode**l yang berfungsi untuk mengelola operasi database terkait data mahasiswa. Kelas ini menyediakan metode untuk:
1. Mengambil semua data mahasiswa (findAll()).
2. Mencari mahasiswa berdasarkan ID (findById()).
3. Menambahkan mahasiswa baru (insert()).
4. Memperbarui data mahasiswa (update()).
5. Menghapus mahasiswa berdasarkan ID (delete()).
Metode-metode tersebut menggunakan **PreparedStatement** untuk query dengan parameter dan **Statement** untuk query tanpa parameter. Kelas ini memetakan hasil query menjadi objek **Mahasiswa** dengan menggunakan **RowMapper**.
...
```
Penjelasan :
Kelas Nilai digunakan untuk menyimpan data nilai mahasiswa, dengan atribut-atribut berikut:
1. id: ID nilai.
2. mahasiswaId: ID mahasiswa yang terkait dengan nilai.
3. mataKuliah: Nama mata kuliah.
4. semester: Semester tempat mata kuliah diambil.
5. nilai: Nilai yang diperoleh mahasiswa.
Kelas ini memiliki konstruktor untuk menginisialisasi atribut-atribut tersebut dan metode getter serta setter untuk setiap atribut, memungkinkan akses dan perubahan nilai-nilai data terkait.
...
```
Penjelasan : 
Kelas NilaiModel mengelola operasi database untuk data nilai mahasiswa. Metode-metode yang disediakan termasuk:
1. findAll(): Mengambil semua data nilai.
2. findByMahasiswaId(int mahasiswaId): Mencari nilai berdasarkan mahasiswa_id.
3. findById(int id): Mencari nilai berdasarkan id.
4. insert(Nilai nilai): Menambahkan nilai baru.
5. update(Nilai nilai): Memperbarui data nilai.
6. delete(int id): Menghapus nilai berdasarkan id.
Kelas ini menggunakan PreparedStatement untuk query dengan parameter dan RowMapper untuk memetakan hasil query menjadi objek Nilai.
...
```
penjelasan : 
Kelas FormMahasiswa adalah tampilan GUI untuk mengelola data mahasiswa menggunakan JFrame. Kelas ini mencakup komponen seperti **input form (JTextField), tombol aksi (JButton), dan tabel (JTable)**. Metode utama mencakup:
1. getMahasiswa(): Mengambil data dari form.
2. setMahasiswas(): Menampilkan daftar mahasiswa di tabel.
3. clearForm(): Mengosongkan form.
4. getSelectedMahasiswaId(): Mengambil ID mahasiswa yang dipilih di tabel.
Kelas ini mendukung operasi CRUD dan memungkinkan interaksi pengguna dengan data mahasiswa.
...
```
Penjelasan :
Kelas FormNilai adalah dialog untuk mengelola data nilai mahasiswa. Kelas ini mencakup:
1. Form input untuk mata kuliah, semester, dan nilai mahasiswa.
2. Tombol aksi untuk menyimpan, menghapus, dan membersihkan data.
3. Tabel untuk menampilkan daftar nilai mahasiswa.
Metode utama mencakup pengaturan informasi mahasiswa, pengambilan dan validasi input nilai, serta menampilkan daftar nilai di tabel. Kelas ini juga mendukung fungsi untuk memilih dan mengelola data nilai mahasiswa.
...
```
Penjelasan :
Kelas Main adalah titik masuk aplikasi yang menginisialisasi koneksi ke database, model, view, dan controller untuk mengelola data mahasiswa. Langkah-langkahnya adalah:
1. Membuka koneksi database.
2. Membuat objek MahasiswaModel untuk berinteraksi dengan database.
3. Membuat objek FormMahasiswa sebagai antarmuka pengguna.
4. Membuat objek MahasiswaController untuk menghubungkan model dan view.
5. Menampilkan tampilan menggunakan view.setVisible(true).
Jika terjadi kesalahan, program akan mencetak error dan keluar.
...
```
Penjelasan : 
Kelas TestConnection digunakan untuk menguji koneksi ke database. Berikut adalah langkah-langkahnya:
1. Mencoba membuka koneksi menggunakan Database.getConnection().
2. Jika berhasil, mencetak pesan "Koneksi berhasil!".
3. Jika terjadi kesalahan, mencetak pesan error dan stack trace.
Setelah berhasil, koneksi ditutup dengan conn.close().

# OutPut FromMahasiswa
![Screenshot 2025-01-14 081101](https://github.com/user-attachments/assets/ec28ea55-7145-49fc-ac9a-162cb61800cf)

# OutPut FromNilai
![Screenshot 2025-01-14 081114](https://github.com/user-attachments/assets/85d7b925-af79-4d34-9911-b3d8ab77f43b)
